...
def assign_images_and_update_jellyfin(json_filename):
    ...
    for item in json_data:
        ...
        if os.path.exists(os.path.join(shows_dir, f"{item_name} ({item_year})")):
            ...
            print(f"Folder not found for item: {item_name} ({item_year}). Skipping.")
            missing_folder = f"{item_name} ({item_year})"
        ...
        if main_poster_path:
            ...
            print(f"Main poster not found for item: {item_name} ({item_year})")
        ...
        for key, image_id in item.items():
            if key.startswith("Season") and image_id:  # Process seasons for shows
                ...
                if not os.path.exists(season_image_path):
                    print(f"Season image not found for item - {item_name} ({item_year}) - {key}")
                    continue

                # Update Jellyfin with season image
                update_jellyfin(image_id, season_image_path, f"{item_name} ({item_year}) - {key}", api_key, jellyfin_url)
                ...
                for image_info in image_id:
                    if 'Type' in image_info and 'Id' in image_info and 'Path' in image_info:
                        ...
                        if not os.path.exists(image_path):
                            print(f"Image not found for item - {item_name} ({item_year}): {image_path}")
                            continue

                        # Update Jellyfin with movie image
                        update_jellyfin(image_id, image_path, f"{item_name} ({item_year}) - {image_type}", api_key, jellyfin_url)
                    else:
                        print(f"Invalid image data found for item - {item_name} ({item_year})")
